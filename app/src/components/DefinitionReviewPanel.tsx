...
/**
 * ⚠️ 2026-09-22 **数值待 XP 体系会话裁决，不要当成已定标准**。
 *
 * 按 `练级与奖励体系方案.md` §2.1 的既有量级，20 是**偏高**的：
 * - 定义题全对 10 / 部分正确 5（学生日常最高难度动作）
 * - 选择 4、拼写 8、学习每 60s 2、连续打卡加成 +10、每日上限 400
 * - 逻辑接龙**一局**（约 1~1.5 分钟）才 20~35
 *
 * 即 20 ≈ **两次定义题全对**，而「提出一次质疑」只是写一两句话。
 * 更要注意**激励扭曲**：若质疑奖励比练习更划算，学生会为拿 XP 大量提质疑，
 * 教师复核负担会随之膨胀 —— 这恰恰是「鼓励质疑」想避免的反面。
 *
 * 合理区间估计在 **5~10**（≈ 一次定义题全对），但**最终数值归 XP 体系会话定**，
 * 本文件只负责「签发一张加分单」，不定义 XP 规则。
 */
