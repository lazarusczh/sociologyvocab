...
> **已实现**：`app/scripts/writing-check.mjs`（2026-09-17）。当前在 **16 段人工范段落上 0 错 0 警**；
> `--selftest` 注入 7 类缺陷（缺题干 / 词条不在库 / 词条未出现在正文 / 动作乱序 / 答案键与标记不符 / 句数不符 / Link 不回题）**全部可检出（7/7）**。
> **首轮战绩**：在编写者本人已核校过的 16 段中仍抓出 **5 处真问题** ——
> `Instinct` 并不在词库（声明与事实不符）；练习 8 引了 `Katz & Lazarsfeld` / `Blumler & McQuail` 却**正文没点名**；
> 两处 Link 未回到题干；一处超长句（46 词）。均已修。

1. **术语白名单**：正文所有学术术语 ∈（建材子集 ∪ 该章 ms 允许的研究名）
2. **词条覆盖**：`uses[]` 每个 term 必须真在 `text` 中出现（词形宽松：允许复数/派生，如 `stereotype`→`stereotypes`、`patriarchy`→`patriarchal`；学者用姓氏；词库里用 `/` 表示的同义写法任一命中即可）
3. **研究名核对**：`facts[].source` 必须 ∈ 该章 ms 清单 → 直接卡死幻觉
4. **动作完整**：`move` 各类 ≥1 且顺序为 P→E→X→L
4b. **答案键自洽**：句子带话语标记时，由标记**反推**顺序并与答案键比对（防手工标注笔误）
...
### 7.2 校验器：`app/scripts/writing-check.mjs`

```bash
cd app
node scripts/writing-check.mjs --md all                       # 扫描项目根下全部 PEEL范段落-*.md
node scripts/writing-check.mjs --md all --dump scripts/out/writing-corpus.json
node scripts/writing-check.mjs --json scripts/out/writing-corpus.json
node scripts/writing-check.mjs --selftest                     # 注入 7 类缺陷，验证检查项本身有效
```

- **正样本**：现有的 16 段人工范段落（两批）。当前 16/16 通过。
- **负样本**：`--selftest` 在内存里对正样本注入 7 类缺陷，逐条断言能被检出 —— 不额外存 fixture 文件。
- **`--dump` 顺带完成格式定型**：把 md 规范化导出为**生成器的目标输出格式**（`sentences[{move,text}]` 按正确顺序 + `uses[]` + `evidence`），
  所以语料格式不必另写规范文档，**dump 出来的就是规范**。
- 词库来自云端证据包（`scripts/out/ev-all.jsonl`，843 概念组 / 208 学者）；学者姓氏复用前端 `scholarSurnames`（同取证器口径）。
...
4. ☑ **校验器**（已完成，见第六节与第七节末）—— 支点：既是生成器质检，也是标签体检
   - ☐ 剩余：C8（位置-标签一致性）的检查已写好，但需要 `scripts/out/writing-labels.json`；标签表未建时自动跳过
